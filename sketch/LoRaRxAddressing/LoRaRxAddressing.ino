...
void setup() 
{
  ...
  Serial.println(LORANET);
  Serial.print("Devices range: ");
  Serial.println(int(pow(2,DEVRANGE)-1));
...
int getMess()  // receive mess from master
{
  int nb;
  int i;
  if ( (nb = LR.receiveNetMess(MYADD,1,(byte*)recbuff,reclen)) <= 0 )
  {
	  delay(10);
	  return 0;
  }
...
void decode(int nb)
{
  ...
  {
	   actBuzzer(val);
	   snprintf(rep,32,"OK Buzzer!");
	   sendReply(rep,strlen(rep));
	   return;
  }
  ...
  {
	  int b = getButt();
	  snprintf(rep,32,"Button: %d !",b);
	  sendReply(rep,strlen(rep));
	  return;
  }
  snprintf(rep,32,"Unrec. cmd: %s",cmd);
  sendReply(rep,strlen(rep));
}
...
void sendReply(char *mess,int messlen)
{
  ...
  Serial.print("> ");
  Serial.println(mess);
  if ( LR.sendNetMess(dest,MYADD,(byte*)sendbuff,messlen) <0 )
...
void sendWarn()
{
  ...
  Serial.print("> ");
  Serial.println(warn);
  if ( LR.sendNetMess(MASTER,MYADD,(byte*)warn,strlen(warn)+1) <0 )
	  Serial.println("Sending error!"); 
  else
	  Serial.println("Sent warn...");
}
