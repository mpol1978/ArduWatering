/* Test LoRa . Use together with LoRaRxEcho receiver sketch to test echo */
...
*  Message is sent at a fixed frequency.
...
* 
*  On serial console are displayed also misured RSSI and SNR values.
*/

#include <LORA.h>
#include <SPI.h>

#define psound 7         // pin for buzzer
//#define pingo 8          // pin for push-button
#define pinf  9          // pin for signaling led
#define MESS "Echo test"    // message to send and receive
...
const int SF = 12;  // Spreading factor value
const int BW = 6;  // Bandwidth value
//int PWR = 4; // transmit power at 50 mW
const int PWR = 5; //transmit power set to 100 mW (20 dBm)
boolean SHIELD = true;

// mpol modify 17-08-16 -- previous time value and send interval time
...
void setup() 
{
  ...
  if ( !LR.begin() ) 
  {
    Serial.println("No LoRa shield detected! Stop!");
    SHIELD = false;
    return;
  }
  ...
  SX.setPower(PWR);
  LR.setConfig(SF,BW,4);  // set different coding rate 1 (def: 9,6,4)
  showConfig();
  strlcpy(inpbuff,MESS,inplen);
  // mpol modify 17-08-16
...
void loop() 
{
  ...
  // mpol modify 17-08-16 -- send data at fixed time interval
  unsigned long currentTime = millis(); // current time in msec
...
void getReplay()
{
  ...
  if (strncmp(recbuff,inpbuff,inc) == 0)
  {
...
void showConfig()
{
  Serial.print("Replay timeout (millisec.): ");
